...
describe("Lock", function () {
  ...
  describe("Testing Saving Contract Deployment", function () {
    it("Should set the right ERC20 token address", async function () {
      const { w3xToken, saveERC20 } = await loadFixture(
        deployERC20TokenAndSavingContractFixture
      );
    ...
    it("Should set the right owner", async function () {
      const { saveERC20, owner } = await loadFixture(
        deployERC20TokenAndSavingContractFixture
      );
  ...
  describe("Saving", function () {
    describe("Validations", function () {
      it("Should revert if zero value is deposited", async function () {
        const { saveERC20 } = await loadFixture(
          deployERC20TokenAndSavingContractFixture
        );

        await expect(saveERC20.deposit(0)).to.be.revertedWithCustomError(
          saveERC20,
      ...
      it("Should revert if deposited amount is greater than user token balance", async function () {
        ...
        await expect(
          saveERC20.deposit(depositAmount)
        ).to.be.revertedWithCustomError(saveERC20, "INSUFFICIENT_BALANCE");
      });
    });

    describe("Events", function () {
      it("Should emit an event on deposit", async function () {
        const { w3xToken, saveERC20, owner } = await loadFixture(
          deployERC20TokenAndSavingContractFixture
        );

        const depositAmount = ethers.parseUnits("1000", 18);

        const approveTx = await w3xToken.approve(
          saveERC20.target,
          depositAmount
        );
    ...
    describe("Withdrawal", function () {
      it("Should withdraw successfully", async function () {
        ...
        const userBalanceBeforeDeposit = await w3xToken.balanceOf(
          owner.address
        );
        ...
        const userTokenBalanceBeforeWithdraw = await w3xToken.balanceOf(
          owner.address
        );
        const contractTokenBalanceBeforeWithdraw =
          await saveERC20.checkContractBalance();

        const withdrawTx = await saveERC20.withdraw(depositAmount);
        await withdrawTx.wait();

        const userTokenBalanceAfterWithdraw = await w3xToken.balanceOf(
          owner.address
        );
        const contractBalanceAfterWithdraw =
          await saveERC20.checkContractBalance();

        await expect(userTokenBalanceAfterWithdraw).to.be.equal(
          userTokenBalanceBeforeWithdraw + depositAmount
        );
        await expect(contractBalanceAfterWithdraw).to.be.equal(
          contractTokenBalanceBeforeWithdraw - depositAmount
        );
      });
    });
  });
